...
// Capriowear's own real public URL, now that it lives at a real
// app/capriowear/ route -- SITE_URL is the bare domain (see content/site.ts),
// this page's own "/capriowear" segment is added here explicitly.
const CANONICAL = `${SITE_URL}/capriowear`;
...
export const metadata: Metadata = {
  // `{ absolute: ... }`, not a plain string -- DEFAULT_TITLE already
  // includes its own "| Capriowear" suffix (owner-supplied exact copy).
  // Before the 2026-09-14 routing move this page sat at the exact same
  // segment as the root layout that owns `title.template`, so the template
  // never applied to it; now that it's nested under /capriowear, `absolute`
  // is what suppresses the template instead, preventing a doubled suffix.
  ...
  alternates: {
    ...
    // ("/capriowear") and metadataBase (the bare domain) don't combine to
    // the right canonical on their own, see content/site.ts.
    canonical: CANONICAL,
  },
  openGraph: {
    title: OG_TITLE,
...
export default function Home() {
  return (
    <>
      ...
      <JsonLd data={breadcrumbSchema([{ name: "Home", url: CANONICAL }])} />
