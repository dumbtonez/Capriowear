...
export async function generateMetadata({
  params,
}: PageProps<"/capriowear/activewear/[category]">): Promise<Metadata> {
  const { category } = await params;
  const data = categories[category];
  if (!data) return {};

  const canonical = `${SITE_URL}/capriowear/activewear/${data.slug}`;
...
export default async function CategoryPage({ params }: PageProps<"/capriowear/activewear/[category]">) {
  const { category } = await params;
  const data = categories[category];
  if (!data) notFound();
