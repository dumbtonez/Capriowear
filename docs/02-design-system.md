...
- **Tablet text-width house rule: a body/subline paragraph directly under a heading gets its own tablet-only width clamp, not just the parent block's flat max-width.** `md:max-w-[clamp(560px,65vw,780px)] xl:max-w-none` — a fluid measure between 560px and 780px, scaling with the viewport rather than a single fixed number, reverting to unconstrained at `xl:` where the parent column's own width already governs the line length. Without it, a heading+subline block whose container is wider at tablet than the reading-comfortable range (roughly 60-75 characters per line) stretches the subline into an unusually long, harder-to-read line. Established 2026-08/09 across `activewearOverview.lead`, `ourServices.lead`, `howItWorks.lead`, `trustSignals.mobileBody`/`itemDescription`, `categoryMetaStrip.lead`, and confirmed again 2026-09-15 on Capriosports' own `capriosportsFactory.lead` (owner: "on tablet, see the house rule for text width which we have decided across the site, should apply this to the entire homepage and pages after this") — apply this by default to any new heading+subline block's own subline, sitewide, not only when it's noticed missing.
  - **Full sitewide audit, both homepages, 2026-09-15:** every heading+subline/body block on `app/page.tsx` (Capriosports) and `app/capriowear/page.tsx` (Capriowear) checked against this rule. Two more real gaps found and fixed: `whyCapriosports.subline` (had only a flat `max-w-[780px]`, stretching to the block's full width at tablet) and `capriosportsFactory.supportingBody` (the 3 supporting cards' body text had no width cap at all below `xl:`, since `supportingCard`'s own `xl:w-[385px]` only starts at desktop). Everything else on both pages was either already compliant or a deliberate, reviewed exception: `whatWeMake.groupBody` and `finalCta.desktopSubline` already carry their own fixed sub-780px caps (620px/623px) that hold at every width they render, so they never actually stretch past the comfortable range even without the fluid clamp; per-card/per-item text in a fixed-width column (`divisionCards.descriptor`, `trustSignals.body`, `capabilityCard.text`/`textDark`, `whyCapriosports.rowBody`) and short captions under a numeric value (`stats.caption`) are a structurally different pattern from a heading+subline block and were left alone.

## PLP section spacing standard (Gear division, established 2026-09-16 on Wraps, Straps & Sleeves)

Applies to every Gear PLP built after this one (`app/lifting-gears/[category]/page.tsx`, `app/boxing-and-mma/[category]/page.tsx`) — a new category should match this by default, not re-derive it per page:

- **Section-to-section boundary, below `xl`: a flat, uniform 72px, every time, with no visible divider line** (owner spec, 2026-09-16: "make 72px each section, keep it consistent" — checked and confirmed already the case on the Weight Lifting Belts PLP via its own untouched `whatWeCover.section`/`trustPoints.sidePaddingPlp`/`fabricOptions.section` recipes, all of which already net a real, single-source 72px gap below `xl`). A new mid-page section (e.g. `ComparisonTable`) gets this via `fabricOptions.sectionFlushBottom`'s own `pt-[72px]`, no `pb`, at mobile/tablet — the section AFTER it supplies the matching gap on its own side (same single-source rule as at `xl`, just a 72px value instead of a divider).
- **Section-to-section boundary, at `xl`: one hairline divider** (`h-px bg-[#e8ecf1]`, in an `mx-auto w-full max-w-[1440px]` wrapper), contributing `xl:mb-[120px]` — the section that follows carries **no top padding of its own** (same "the divider is the sole source of the gap" rule `whatWeCover.section` already established; a section's own `pt` alongside the divider double-counts the gap). Used under the product grid/pagination and between any two stacked optional sections (e.g. the comparison table and the style listing).
- **A section that should sit flush against its neighbour with NO visible divider at `xl`** still needs a real 120px via its own bottom padding, not by leaving a divider in place and hiding it — `fabricOptions.sectionFlushBottom` is the recipe for "top gap comes from the divider above (or the standard 72px below `xl`), bottom gap is this section's own `xl:pb-[120px]`/mobile `pt-[72px]`, no line drawn on either side."
- **Section title (H2) to its own content below it (table, grid, etc.), within the SAME section = 72px at every breakpoint** (owner spec, 2026-09-16) — `fabricOptions.weightTiersWrapWide`, not the original `weightTiersWrap` (`44px`/`60px`, still correct and untouched for every pre-existing Activewear/Teamwear/Gear category's own secondary fabric table) and not the desktop-only `xl:mt-[120px]` used between multiple stacked tables in the same section (`SpecTables.tsx`, where a genuinely different, explicitly requested 120px value applies only at `xl:`; below `xl` those inter-table gaps are the same flat 72px as everywhere else on the page).
- **When comparing two gaps to check they "match," measure element BOX edges (`getBoundingClientRect()` on the actual padded/margined wrapper), never TEXT NODE positions** — two visually-adjacent gaps that are byte-identical in CSS can still measure a different number of pixels apart if one side's last visible line is a different kind of element (a `<td>` cell vs. a `<p>` paragraph, e.g.), since each element's own line-height/descender space adds to where its text visually sits within its own box. A same-CSS gap that "measures" 97px on one side and 72px on the other is not a real inconsistency — re-verify at the box level before concluding a value needs to change (real mistake made once on this page: `SpecTables.tsx`'s inter-table margin was bumped from `72px` to `97px` to chase a text-position measurement that was already correct at the box level).
- **Never override an existing padding/margin utility by appending a conflicting one to the same className string** (e.g. `cx(existingRecipe, "xl:pt-0")` to cancel out that recipe's own `xl:pt-[80px]`) — Tailwind's generated CSS order is independent of the order classes appear in the `className` attribute, so the original value can silently keep winning even though the override reads correctly in the JSX (a real bug, found live on this page: measured 200px where the code implied 120px). Add a genuinely separate, fully-spelled-out recipe key instead, as `sectionFlushBottom`/`weightTiersWrapWide` do here.
