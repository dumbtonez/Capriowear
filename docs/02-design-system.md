...
- **Section-to-section boundary = one hairline divider** (`h-px bg-[#e8ecf1]`, in an `mx-auto w-full max-w-[1440px]` wrapper), contributing `xl:mb-[120px]` — the section that follows carries **no top padding of its own** (same "the divider is the sole source of the gap" rule `whatWeCover.section` already established; a section's own `pt` alongside the divider double-counts the gap). Used under the product grid/pagination and between any two stacked optional sections (e.g. the comparison table and the style listing).
- **A section that should sit flush against its neighbour with NO visible divider** still needs a real 120px (or 0px, if genuinely flush both sides) via its own bottom padding, not by leaving a divider in place and hiding it — `fabricOptions.sectionFlushBottom` is the recipe for "top gap comes from the divider above, bottom gap is this section's own `xl:pb-[120px]`, no line drawn on either side."
- **Section title (H2) to its own content below (table, grid, etc.) = 72px** at `xl:` (mobile/tablet keeps the existing smaller `mt-[44px]`) — `fabricOptions.weightTiersWrapWide`, not the original `weightTiersWrap` (`60px`, still correct and untouched for every pre-existing Activewear/Teamwear/Gear category's own secondary fabric table).
