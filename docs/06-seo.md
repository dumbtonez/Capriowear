...
The site is staging on `capriowear.vercel.app` ahead of the Phase 5 cutover to `www.capriosports.com` (see above) and must not be indexed by Google until then. One env var controls the whole site: `NEXT_PUBLIC_ALLOW_INDEXING`, read into `content/site.ts`'s `ALLOW_INDEXING` constant (defaults to `false`/off whenever unset). While off: `app/layout.tsx` renders a sitewide `<meta name="robots" content="noindex, nofollow">` (via the Metadata API's own `robots` field, no per-page override reintroduces indexing), and `app/robots.ts` disallows every path for every agent, with no sitemap reference. Since 2026-09-26 (audit 2026-09, B-07) the lock also has a header form: while off, `next.config.ts` sends `X-Robots-Tag: noindex, nofollow` on **every** response (images, PDFs and OG PNGs included, which a meta tag can't reach). Independently of the switch, `X-Robots-Tag: noindex, nofollow` is always sent on /studio, /styleguide, /api/* and on any `*.vercel.app` host, so the production alias can never compete with www after launch. Set `NEXT_PUBLIC_ALLOW_INDEXING=true` in Vercel's production environment variables to flip all of them at once, no code change needed, and only after the cutover's redirect map is live.
...
- **Builders**: `lib/schema.ts` — `organizationSchema()`, `websiteSchema()`, `breadcrumbSchema(items)`, `faqSchema(items)`, `navigationSchema(links)`, `megaMenuSchema(name, href, groups)`, `collectionPageSchema(name, url, description, items)`. Each is a pure function; `breadcrumbSchema`/`faqSchema`/`navigationSchema`/`megaMenuSchema`/`collectionPageSchema` read from whatever a page already renders. `organizationSchema()`/`websiteSchema()` read from `content/capriosports/organization.ts`'s `CAPRIOSPORTS_ORGANIZATION` (the sitewide root entity is Capriosports, the parent company, with Capriowear represented as a `subOrganization` node off `content/site.ts`'s own `ORGANIZATION` — 2026-09-14/15 Capriosports parent-site task) — not `content/site.ts` directly, as an earlier pass of this doc said.
- **Rendering**: `components/JsonLd.tsx` — one prop, `data`, serializes it into a `<script type="application/ld+json">`, with `<` escaped as `\u003c` so no value can close the tag (2026-09-26). Never build the script tag by hand elsewhere.
...
- **PDP Product schema** (`productSchema()`, emitted only for a published style, 2026-09-24): `name` is the style name (card title, never the "... Manufacturer" H1), plus `url` (canonical PDP URL), `category` (the category label, e.g. "Hoodies"), `description`, `brand`, `manufacturer` (name "Capriosports", the schema register; the Organization node has no `@id` to reference), `material`, `sku`, and `image` only when a real product photo exists. Never `offers` or a price. The Capriowear `subOrganization` carries `companyIntroShort` as its `description`. The Capriowear PLPs' CollectionPage description reads "`[Category]` by Capriowear. `[companyIntroShort]`".
- **Publish readiness** (2026-09-25, Jumpsuits audit #13): `isPublished()` in `content/activewear/pdpShared.ts` is true only when `status` is `"published"` AND `getPublishReadiness()` passes: `sku`, `pdpHeading`, a meta title (`pdpMetaTitle` or the derived `Custom [pdpTitle ?? cardTitle] Manufacturer`), `pdpMetaDescription`, `pdpDescription`, at least one `specifications` row, at least one per-style `faqs` entry, and at least one `images[].src` that isn't a placeholder (`isRealImageSrc()`: not placehold.co, `/factory-test/`, or `/Product%20images/`). A published style that fails renders exactly like a draft (noindex/nofollow, no Product or FAQPage JSON-LD, out of the sitemap and the PLP ItemList) and logs one `[publish-readiness]` server warning naming the SKU and the missing fields. All four divisions' PDP and PLP routes gate on `isPublished()`, while route reachability still reads `status`. Build-time backstop: `assertPublishedStylesReady()`, called at the top of `app/sitemap.ts`, fails `next build` with a list of every offending style. It only runs during the production build phase.
- **Product `material`** (2026-09-25, Jumpsuits audit #23): `productSchema()` emits `schemaMaterial` if set, otherwise `plainMaterial(material)`. That strips sample-confirmation clauses, "commonly/roughly around" blend ranges, ratio clauses and parentheticals, GSM weights, approximate percentages and the trailing period, but keeps a definitive single-fiber "100%" ("100% polyester"), so the schema carries only the material ("Nylon/Spandex 4-way stretch knit"). The visible Fabric spec row is untouched. `StyleCard.schemaMaterial` is only for values the helper can't reduce cleanly (today: T-Shirts "50/50 or CVC", Long-Sleeve Tops "at least 75% recycled").
- **PDP OG image**: dynamic `[style]` segments don't inherit `app/opengraph-image.tsx`, so the activewear and teamwear PDP routes re-export it from their own `opengraph-image.tsx` (same pattern as the PLP's `[category]/opengraph-image.tsx`), until real product photography exists.
- **Entity naming for GEO**: the canonical entity intro is locked at four lengths and two naming registers (owner spec, 2026-09-16): "Capriosports" (one word) only in schema/metadata contexts (`<title>`, `<meta name="description">`, JSON-LD `Organization` `legalName`/`description`); "Caprio" alone in all visible frontend copy. Stored once, never retyped:
  - `content/capriosports/organization.ts`'s `CAPRIOSPORTS_ORGANIZATION.identityLine.gear` (schema register one-liner) feeds `organizationSchema()`'s `Organization.description` sitewide; `.gearFrontend` (same one-liner, "Caprio") is the frontend-register equivalent for visible copy (the Capriosports/Gear footer, etc.).
  - `.introShort.schema`/`.frontend` — the 3-sentence "who we are" text, used where there's no hard character cap.
  - `.metaIntro` — 152 chars, schema register, the only variant sized to fit inside an actual `<meta name="description">` without truncating; feeds the homepage's own meta description (`content/capriosports/home.ts`).
  - The full 4-sentence PLP/PDP entity-FAQ template ("What does Caprio manufacture?") is built per category, not a stored constant — see `buildCaprioEntityAnswer()`/`categoryEntityFaq()` in `content/activewear/pdpShared.ts`, the automatic default for any `Category` with `group: "Gear"`, always ending in `identityLine.gearFrontend` plus the standard "Capriowear, our activewear and teamwear division, is built in the same facility." sentence. Capriowear's own copy (`content/site.ts`'s `companyIdentity`/`companyIntroShort`/`companyIntroMeta`) is the separate, untouched equivalent for the Capriowear brand itself.
- **Certifications**: `content/site.ts`'s `CERTIFICATIONS` const (ISO 9001, ISO 45001, ISO 14001, BSCI, IMAC, SGS — 6 items, owner-confirmed 2026-09-11) is the single source of truth for every visible mention of certifications sitewide (homepage trust strip/FAQ, Our Factory trust strip/FAQ, Services FAQ, Teamwear hub trust strip) and feeds `organizationSchema()`'s `hasCredential` array (one minimal `EducationalOccupationalCredential` per cert). A short trust-strip bullet can word it naturally for its context, but the underlying set must always match this const — never a second, independently-typed list. No CE, no PSGMEA/PRGMEA membership claims, and **no OEKO-TEX** (Capriowear does not hold it — confirmed with the team 2026-09-11, after it had been wrongly treated as held in nearly every content doc up to that point) — do not reintroduce any of these.
- **Memberships** (distinct from certifications): `content/site.ts`'s `MEMBERSHIPS` const (WFSGI, confirmed real) feeds `organizationSchema()`'s `memberOf` (an `Organization` entity per membership), never `hasCredential` — a membership is belonging to an industry body, not a third-party audit of the factory. "SGI" is **not** a real certification; it was a same-day owner mix-up with WFSGI (whose own logo mark visually renders "SGI" as its large center letters) caught and corrected 2026-09-11 — don't reintroduce it.
