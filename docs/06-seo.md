...
- **Certifications**: `content/site.ts`'s `CERTIFICATIONS` const (ISO 9001, ISO 45001, ISO 14001, BSCI, IMAC, SGS, OEKO-TEX — 7 items, owner-confirmed 2026-09-11) is the single source of truth for every visible mention of certifications sitewide (homepage trust strip/FAQ, Our Factory trust strip/FAQ, Services FAQ, Teamwear hub trust strip) and feeds `organizationSchema()`'s `hasCredential` array (one minimal `EducationalOccupationalCredential` per cert). A short trust-strip bullet can word it naturally for its context, but the underlying set must always match this const — never a second, independently-typed list. No CE, and no PSGMEA/PRGMEA membership claims (dropped 2026-09-11 as false).
